...
export class Api_hygge {
  ...
  updateImageProfile(db: Knex, cid: any) {
    return db('hygge_citizen').where('cid', cid).update({ image_path: cid });
  }

  updateDeviceTokenToNull(db: Knex, cid: any) {
    return db('hygge_citizen').where('cid', cid).update({ device_token: null });
  }
